...
public class CustomJpaEntityProviderDistTest {
    ...
    void dbKindSpecifiedInBuildTime(KeycloakDistribution dist) {
        ...
        result.assertMessage("You have set DB kind for 'client-store' datasource via an unquoted Quarkus property 'quarkus.datasource.client-store.db-kind'. All datasource options must have the datasource name quoted to be compatible with Keycloak's db-* options.");
        result.assertBuild();
