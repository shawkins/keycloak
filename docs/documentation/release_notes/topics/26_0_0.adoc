...
= `X-Forwarded-Prefix` will be considered 

When using `proxy-headers=xforwarded` the `X-Forwarded-Prefix` header will be considered, this may allow you to not use a full url for the `hostname` option.
