...
The context path of {project_name} can be changed using the `http-relative-path` option.

If the proxy context path is different than {project_name} server, you can use a full URL for the `hostname` option. For example using `--hostname=https://my.keycloak.org/auth` if {project_name} is exposed through the reverse proxy on `/auth`.

Alternatively if you are using `xforwarded` proxy headers, and the proxy is setting the `X-Forwarded-Prefix` header, then you can continue to use just a simple hostname.
