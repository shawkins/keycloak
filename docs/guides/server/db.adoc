...
+
The minimum settings needed to connect to the database can be specified via the cli, environment variables, or shown here in `conf/keycloak.conf`:
+
----
# The database vendor.
db=postgres
...
On the server start command, the options will be used to automatically build the proper augmentation:
+
----
bin/kc.[sh|bat] start
----
...
WARNING: Do not directly pass the database password or other sensitive values via the cli. Use the `conf/keycloak.conf` as shown above, environment variables, or a config keystore for at least the password.
